...
const char* ASSET_DIRS[]  = {"", "cally", "paladin", "skeleton"};
...
void Engine::LoadAssets() {
    LOGI("data dir:%s", app_->activity->internalDataPath);

    AAssetManager* assetMgr = app_->activity->assetManager;
    for (int i = 0; i < 4; i++) {
        //mkdir first
        if (i > 0) {
            string assetDir = string(app_->activity->internalDataPath) + "/" + ASSET_DIRS[i];
            if (!mkdir(assetDir.c_str(), 0770)) {
                LOGW("mkdir [%s] failed.", assetDir.c_str());
            }
        }

        AAssetDir* assetDir = AAssetManager_openDir(assetMgr, ASSET_DIRS[i]);
        const char* filename = NULL;
        while ((filename = AAssetDir_getNextFileName(assetDir)) != NULL) {
            string assetName = i == 0 ? string(filename) : string(ASSET_DIRS[i]) + "/" + filename;
            LOGI("assetName:%s", assetName.c_str());
            AAsset* asset = AAssetManager_open(assetMgr, assetName.c_str(), AASSET_MODE_STREAMING);;

            char buf[BUFSIZ];
            int nb_read = 0;
            string savePath = string(app_->activity->internalDataPath) + "/" + assetName;
            LOGI("savePath:%s", savePath.c_str());
            FILE* out = fopen(savePath.c_str(), "w");
...
//    renderer_.UpdateViewport();
    demo_->setDimension(gl_context_->GetScreenWidth(), gl_context_->GetScreenHeight());
